...
echo "# Intro: http://www.google.com"
echo "#"
echo "# Author: Google"
echo "#"
echo "#############################################################"
...
function pre_install(){
    ...
    read -p "(Default password: sodans0cks~r):" shadowsockspwd
    if [ "$shadowsockspwd" = "" ]; then
        shadowsockspwd="sodans0cks~r"
    fi
...
{
    "server":"74.82.188.244",
    "server_port":8989,
    "local_address":"127.0.0.1",
    "local_port":1080,
    "password":"sodans0cks~r",
    "timeout":600,
    "method":"aes-256-cfb"
}
EOF
...
function install(){
    ...
    if [ -s /usr/local/bin/ss-server ];then
        ...
        if [ $? -eq 0 ]; then
            ...
            echo ""
            echo "Shadowsocks-libev install failed! Please visit http://www.google.com and contact."
            exit 1
        fi
    fi
    cd $cur_dir
